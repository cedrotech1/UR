...
if (isset($_POST['day'])) {
  ...
  if ($report == 'stockin') {
    ...
    $ok = mysqli_query($connection, "SELECT product.pid,product.pname,historic.quantity,historic.price,historic.total,historic.year,historic.month,historic.day,historic.time,historic.profit
          FROM product,historic WHERE product.pid=historic.pid and status='stockin' and day='$day' and year='$year' and month='$month' order by id desc");
    while ($row = mysqli_fetch_array($ok)) {
      $total_cost = $total_cost + $row["4"];
      $total_profit = $total_profit + $row["9"];
    }
    ...
    $ok = mysqli_query($connection, "SELECT product.pid,product.pname,historic.quantity,historic.price,historic.total,historic.year,historic.month,historic.day,historic.time,historic.profit
    FROM product,historic WHERE product.pid=historic.pid and status='stockout' and day='$day' and year='$year' and month='$month' order by id desc");
while ($row = mysqli_fetch_array($ok)) {
$ts = $ts + $row["4"];
$total_profit = $total_profit + $row["9"];
}
...
<body>
  ...
  <main id="main" class="main">
                    ...
                    <div class="row">
                      <div class="col-md-12 table-responsive">
                        ...
                        if ($status == 'stockin') {
                          ...
                          ?>
                                 <ul style='margin-left:-1cm'>
                               <li>total selling : <b> <?php echo formatMoney($ts); ?></b></li>
                               <li>total cost :<b> <?php echo formatMoney($total_cost);?></b></li>
                               <li>total profit:<b> <?php echo formatMoney($total_profit); ?></b></li>
    ...
    <div class="button-container">
        <button class="btn btn-primary" onclick="exportPDF()">Download as PDF</button>
        <!-- <button  class="btn btn-info" onclick="exportExcel()">Download as Excel</button> -->
    </div>
...
<script>
        async function exportPDF() {
            ...
            // Capture the content of the division
        ...
        function exportExcel() {
            ...
            const heading = content.querySelector('h1').innerText;
            const paragraph = content.querySelector('p').innerText;
            worksheetData.push([heading]);
            worksheetData.push([paragraph]);
