...
function getCountByColumn($connection, $column) {
    $query = "SELECT `$column`, COUNT(*) as count FROM info GROUP BY `$column`";
...
function getCountWithoutImagesByColumn($connection, $column) {
    $query = "SELECT `$column`, COUNT(*) as count FROM info WHERE `picture` IS NULL OR `picture` = '' GROUP BY `$column`";
    $result = mysqli_query($connection, $query);
    $data = [];
...
function getTotalWithoutImages($connection) {
    $query = "SELECT COUNT(*) as total FROM info WHERE `picture` IS NULL OR `picture` = ''";
    $result = mysqli_query($connection, $query);
    $row = mysqli_fetch_assoc($result);
    return $row['total'] ?? 0;
}
